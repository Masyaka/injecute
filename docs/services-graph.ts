...
function buildConnections(graph: Tree) {
  return Object.entries(graph).flatMap(([nodeId, node]) => {
    if (!node || !node.dependencies) return [];
    return Object.keys(node.dependencies).map(dependency => ({
      from: nodeId,
      to: dependency,
    }));
  });
...
export function renderConnections(graph: Tree) {
  const container = document.querySelector('#tree-container-svg');
  if (!container) {
    throw new Error('Container not found');
  }
  const connections = buildConnections(graph);
